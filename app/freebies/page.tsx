    ...
    <main className="freebies-page min-h-screen overflow-hidden bg-[#f4f6f8] text-[#111318]">
      ...
      <section className="freebie-factory-section relative overflow-hidden border-y border-black/10 bg-[#0b1120] px-[7vw] py-24 text-white">
        <div className="freebie-factory-grid" /><div className="relative z-10 grid items-center gap-14 lg:grid-cols-[.72fr_1.28fr]">
          <div><p className="font-mono text-[8px] tracking-[.18em] text-[#70a7ff]">02 / LIVE TOOL ENGINE</p><h2 className="mt-6 text-[clamp(48px,6vw,88px)] font-medium leading-[.84] tracking-[-.08em]">Watch a file<br /><span className="text-[#70a7ff]">transform.</span></h2><p className="mt-8 max-w-[470px] text-sm leading-7 text-white/45">Drop in a file, let the browser do the work, and leave with the result.</p></div>
      ...
      <section id="free-tool-list" className="freebies-tools-section border-t border-black/10 bg-white/55 px-[7vw] py-24">
        <div className="mb-14 flex flex-col justify-between gap-6 lg:flex-row lg:items-end"><div><p className="font-mono text-[8px] tracking-[.16em] text-black/35">03 / CONVERTERS</p><h2 className="mt-6 text-[clamp(50px,6vw,90px)] font-medium leading-[.86] tracking-[-.08em]">Convert it.<br /><span className="text-[#2563ff]">Get it done.</span></h2><p className="mt-7 max-w-[560px] text-sm leading-7 text-black/45">Everyday image and PDF converters from ConvertX, launched from one free browser workspace.</p></div><div className="freebies-count-box"><strong>10</strong><span>CONVERTERS</span><small>POWERED BY CONVERTX</small></div></div>
      ...
      <section id="academic-tools" className="border-t border-black/10 bg-[#eef4ff] px-[7vw] py-24">
        <div className="mb-14 flex flex-col justify-between gap-6 lg:flex-row lg:items-end"><div><p className="font-mono text-[8px] tracking-[.16em] text-[#2563ff]">04 / ACADEMICS</p><h2 className="mt-6 text-[clamp(50px,6vw,90px)] font-medium leading-[.86] tracking-[-.08em]">School work.<br /><span className="text-[#2563ff]">Simplified.</span></h2><p className="mt-7 max-w-[560px] text-sm leading-7 text-black/45">Academic tools from TechCraft, starting with Gradly for fast and clean student report-card generation.</p></div><div className="freebies-count-box"><strong>01</strong><span>ACADEMIC TOOL</span><small>FROM TECHCRAFT</small></div></div>
        <div className="max-w-[720px]"><a href={gradlyUrl} target="_blank" rel="noopener noreferrer" aria-label="Open Gradly school report card generator" className="freebies-tool-card block text-inherit no-underline"><div className="freebies-tool-card-top"><span>01</span><span className="freebie-status"><Check size={11} /> FREE</span></div><div className="freebies-tool-icon"><GraduationCap size={21} /></div><p className="mt-7 font-mono text-[7px] tracking-[.16em] text-[#2563ff]">SCHOOL REPORT CARDS</p><h3>Gradly</h3><p className="freebies-tool-description">A school report card generator for creating student results with marks, grades, totals, percentages and academic summaries.</p><div className="freebies-tool-footer"><span>ACADEMIC</span><span className="freebies-tool-action">OPEN GRADLY <ArrowRight size={13} /></span></div></a></div>
      </section>
      ...
      <section className="freebies-cta border-t border-black/10 bg-[#111318] px-[7vw] py-24 text-white"><div className="flex flex-col justify-between gap-10 lg:flex-row lg:items-end"><div><p className="font-mono text-[8px] tracking-[.16em] text-[#70a7ff]">06 / KEEP IT FREE</p><h2 className="mt-6 max-w-[850px] text-[clamp(48px,6vw,86px)] font-medium leading-[.87] tracking-[-.08em]">Small tools can solve <span className="text-[#70a7ff]">big annoyances.</span></h2></div><a href="#free-tool-list" className="techcraft-button techcraft-button-primary">Explore the toolkit <ArrowRight size={16} /></a></div></section>
    </main>
  );
}
